...
class Injector(nn.Module):
    ...
    def inject(self, x: torch.Tensor, p: float) -> torch.Tensor:
        ...
        if self.training:
            error = self.training_error(self.current_epoch)
            error_model = self.error_model
        else:
            error = self.random_relative_error
            error_model = np.random.choice(self.mask_types)

        mask = generate_mask(x, error_model, p)
        x[mask] = x[mask].mul_(error)
    ...
    def training_error(self, epoch):
        error = torch.rand(size=(1,), device=self.device) * 6 + 1e-6  #* max(1, epoch, 1)
...
def generate_line_mask(h, w):
    ...
    if torch.bernoulli(torch.ones(1) * .5):
        # columns
        column = torch.randint(0, w, (1,))
        mask[column, :] = 1
    else:
        # rows
        row = torch.randint(0, h, (1,))
        mask[:, row] = 1
    return mask
